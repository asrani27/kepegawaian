    ...
    <div class="col-12">
        <div class="card card-outline card-primary">
            <div class="card-header">
                <h3 class="card-title">EDIT DATA JENJAB</h3>
            ...
            <!-- /.card-header -->
            <form method="post" action="/kepegawaian/kelola/jenjab/{{$data->id}}/edit">
                @csrf
                <div class="card-body">
                    <div class="form-group">
                        <label>NAMA JENJAB</label>
                        <input type="text" class="form-control" name="nama_jenjab" value="{{$data->nama_jenjab}}"
                            required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="keljab_id" style="width: 100%;" required>
                            ...
                            <option value="{{$item->id}}" {{$data->keljab_id==$item->id ?
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="min_pangkat_id" style="width: 100%;" required>
                            ...
                            <option value="{{$item->id}}" {{$data->min_pangkat_id==$item->id ?
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="max_pangkat_id" style="width: 100%;" required>
                            <option value="">-pilih-</option>
                            @foreach ($pangkat as $item)
                            <option value="{{$item->id}}" {{$data->max_pangkat_id==$item->id ?
                                'selected':''}}>{{$item->pangkat}}</option>
                            @endforeach
                        </select>
                    </div>
